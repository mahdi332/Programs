...
def generate_successors(node):
    ...
    if node.position_agent == 0:
        ...
        if node.state[node.position_agent] == "D":
            ...
            successors.append(
                Node(
                    node.position_agent, s, parent=node, action="Suck", d=node.depth + 1
                )
            )
        ...
        print(
            ...
            " = ",
            node.state,
            ", there are at most 2 actions Left and Suck.",
        )
        print("Run the actions and add new states to the edd of successors.")
        successors.append(
            Node(
        ...
        print(
            ...
            " = ",
            node.state,
            ", there are at most 3 actions Left, Right and Suck.",
        )
        print("Run the actions and add new states to the edd of successors.")
        successors.append(
            Node(
...
def dls_vacuum_cleaner(environment, initial_position, limited_depth):
    ...
    while frontier:
        ...
        for i in frontier:
            print(i.position_agent, i.state, end=", ")
        print()
        ...
        if ([current_node.position_agent, current_node.state]) in ES:
            ...
            print(
                current_node.position_agent,
                " agent in",
                current_node.state,
                " is visited. Go to the next order.",
            )
            ...
            print(
                ...
                " agent in",
                current_node.state,
                " has reached a limited depth and becomes a leaf.",
            )
        else:
        ...
        print(
            "Teminate an iteration -----------------------------------------------------"
        )
    else:
        print(
            "The problem in the limited depth",
            limited_depth,
            " could not reach to the goal!",
        )
        return solution


environment = ["D", "D"]
max_depth = int(input("Enter maximum depth for Interative Deepening Search : "))
for limited_depth in range(1, max_depth + 1):
    if dls_vacuum_cleaner(environment, 0, limited_depth):
        print("Algorithm reaches to solution in depth ", limited_depth)
        break
    else:
        print("\n\n\nAlgorithm could not reach to solution in depth ", limited_depth)
        print("So, it will make the search one unit deeper.")
else:
    print("\n\nIDS with maximum depth ", max_depth, " could not reach to solution!")
