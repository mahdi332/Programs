...
def generate_successors(node, romania_map, Heuristic):
    ...
    for i in romania_map[node.name]:        
        successors.append(Node(i, Heuristic, parent=node))    
...
def greedy_RomaniaTurism(romania_map, initial_city, Heuristic):
    initial_state = Node(initial_city, Heuristic)  # Start from initial city
    frontier = [initial_state]
    Eval_n = [initial_state.Eval]
    ES = []
    while frontier:
        print('frontier is | ', end=' ')  # print frontier
        for i in frontier: print( i.name, ', Eval=', i.Eval, end='| ')
        print('\nE(n) is ', end=' ')   # print E(n) 
        for i in Eval_n: print( i, end=', ')      
        current_node = frontier.pop(Eval_n.index(min(Eval_n)))    # Selection based on the lowest E(n)
        Eval_n.pop(Eval_n.index(min(Eval_n)))    # remove the lowest E(n)
        print('\nES = ', ES)
...
greedy_RomaniaTurism(romania_map, 'Arad', Heuristic)
